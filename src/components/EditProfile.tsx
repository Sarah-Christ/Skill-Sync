...
setName(data.name || "");
setSkillsToLearn(
Array.isArray(data.skillsToLearn)
? data.skillsToLearn
: typeof data.skillsToLearn === "string"
? data.skillsToLearn.split(",").map((s: string) => s.trim())
: []
);
setSkillsToOffer(
Array.isArray(data.skillsToOffer)
? data.skillsToOffer
: typeof data.skillsToOffer === "string"
? data.skillsToOffer.split(",").map((s: string) => s.trim())
: []
);
}
}
    ...
    {showLearnInput ? (
      ...
      <div className="input-row">
        ...
        <div
          ...
          onDrop={handleDrop}
          title="Drag a skill here to delete"
        >
          🗑️
        </div>
      </div>
